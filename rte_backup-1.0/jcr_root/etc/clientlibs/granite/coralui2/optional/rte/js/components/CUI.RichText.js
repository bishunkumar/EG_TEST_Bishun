    ...
    CUI.RichText = new Class(/** @lends CUI.RichText# */ {
        ...
        _handleToolbarOnSelectionChange: function() {
            ...
            if (com.ua.isTouch) {
                ...
                var _lastSel;
                $doc.on("selectionchange.rte-toolbarhide-" + this.id, function(e) {
        ...
        handleKeyUp: function(e) {
            ...
            if (e.getCharCode() === 27) {
                if (!this._dispatchEvent("beforeEscape")) {
                    this.finish(true);
                }
            }
        },
        ...
        initializeEventHandling: function() {
            ...
            /*
            $uiBody.fipo("tap.rte-item", "click.rte-item", ".coral-RichText-popover .item",
                    function(e) {
                        self.isTemporaryFocusChange = true;
                        CUI.rte.UIUtils.killEvent(e);
                    });
        ...
        finalizeEventHandling: function() {
            if (this.editorKernel != null) {
                ...
                $body.off("touchstart.rte-ooa-" + this.id
                        + " mousedown.rte-ooa-" + this.id);
                // Toolbar
                $uiBody.off("tap.rte-item-" + this.id + " click.rte-item-" + this.id);
                $uiBody.off("mousedown.rte-item-" + this.id);
                        ...
                        function(plugin, feature) {
                            if (ac) {
                                ...
                                if (plugin === "control") {
                                    ...
                                    var fsf = (isFullScreen ? "finish" : "start");
                                    return (feature === fsf);
                                }
                            }
                            return undefined;
