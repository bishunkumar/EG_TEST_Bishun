    ...
    execute: function(execDef) {
        ...
        if (htmlToInsert && (htmlToInsert.length > 0)) {
            if (com.ua.isIE) {
                ...
                // to insert the HTML
                try {
                    var range = CUI.rte.Selection.saveNativeSelection(execDef.editContext);
                    if (range.pasteHTML) {
                        range.pasteHTML(htmlToInsert);
                    } else {
                        var tempDiv = execDef.editContext.doc.createElement("div");
                        tempDiv.innerHTML = htmlToInsert;
                        var textFrag = execDef.editContext.doc.createDocumentFragment();
